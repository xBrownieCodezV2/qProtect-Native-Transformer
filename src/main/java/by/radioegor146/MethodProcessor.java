...
public class MethodProcessor {
    ...
    private NativeTransformer nativeTransformer;
    private InstructionHandlerContainer<?>[] handlers;

    public MethodProcessor(NativeTransformer nativeTransformer) {
    ...
    public void processMethod(MethodContext context) {
        ...
        if (Util.getFlag(context.clazz.access, Opcodes.ACC_INTERFACE)) {
            ...
            String methodSource = String.format("            { (char *)%s, (char *)%s, (void *)&%s },\n",
                    nativeTransformer.getStringPool().get(outerJavaMethodName),
                    nativeTransformer.getStringPool().get(targetDesc), methodName);

            nativeTransformer.getStaticClassProvider().addMethod(context.nativeMethod, methodSource);
        } else {
            context.nativeMethods.append(String.format("            { (char *)%s, (char *)%s, (void *)&%s },\n",
                    nativeTransformer.getStringPool().get(context.proxyMethod.name),
                    nativeTransformer.getStringPool().get(method.desc), methodName));
        }
        ...
        for (int i = 0; i < context.argTypes.size(); ++i) {
            ...
            output.append("    ").append(nativeTransformer.getSnippets().getSnippet(
